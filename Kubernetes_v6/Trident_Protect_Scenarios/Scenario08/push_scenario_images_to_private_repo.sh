...
if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/wordpress/tags/list' | jq -r '.tags[]? | select(.=="6.2.1-apache")')" ]]; then
  echo
  echo "##############################################################"
  ...
  podman run --rm quay.io/containers/skopeo:latest copy --dest-creds 'registryuser:Netapp1!' \
    docker://quay.io/yvosonthehub/wordpress:6.2.1-apache docker://registry.demo.netapp.com/wordpress:6.2.1-apache \
    --src-tls-verify=false --dest-tls-verify=false 
else
  echo
  echo "##############################################################"
  echo "# Wordpress 6.2.1 already in the Private Repo - nothing to do"
  echo "##############################################################"
fi

if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/mysql/tags/list' | jq -r '.tags[]? | select(.=="8.0")')" ]]; then
  echo
  echo "##############################################################"
  echo "# Skopeo Copy Mysql 8.0 Into Private Repo"
  echo "##############################################################"
  podman run --rm quay.io/containers/skopeo:latest copy --dest-creds 'registryuser:Netapp1!' \
    docker://quay.io/yvosonthehub/mysql:8.0 docker://registry.demo.netapp.com/mysql:8.0 \
    --src-tls-verify=false --dest-tls-verify=false 
else
  echo
  echo "##############################################################"
