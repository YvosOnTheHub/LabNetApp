...
if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/busybox/tags/list' | jq -r '.tags | map(test("site")) | any')" ]]; then
  echo
  echo "##############################################################"
  ...
  podman run --rm quay.io/containers/skopeo:latest copy --dest-creds 'registryuser:Netapp1!' \
    docker://quay.io/yvosonthehub/busybox:1.35.0 docker://registry.demo.netapp.com/busybox:site1 \
    --src-tls-verify=false --dest-tls-verify=false 

  podman run --rm quay.io/containers/skopeo:latest copy --dest-creds 'registryuser:Netapp1!' \
    docker://quay.io/yvosonthehub/busybox:1.35.0 docker://registry.demo.netapp.com/busybox:site2 \
    --src-tls-verify=false --dest-tls-verify=false 
else
  echo
  echo "##############################################################"
