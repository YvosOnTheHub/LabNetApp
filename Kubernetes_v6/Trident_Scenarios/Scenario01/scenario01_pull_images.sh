...
RATEREMAINING=$(curl --head -H "Authorization: Bearer $TOKEN" https://registry-1.docker.io/v2/ratelimitpreview/test/manifests/latest 2>&1 | grep -i ratelimit-remaining | head -n 1 | cut -d ':' -f 2 | cut -d ';' -f 1 | xargs)
...
if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/trident/tags/list' | jq -r '.tags[]? | select(.=="26.02.1")')" ]]; then
  ...
  docker://quay.io/yvosonthehub/netapp/trident:26.02.1 docker://registry.demo.netapp.com/trident:26.02.1 \
...
if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/trident-operator/tags/list' | jq -r '.tags[]? | select(.=="26.02.1")')" ]]; then
  ...
  docker://quay.io/yvosonthehub/netapp/trident-operator:26.02.1 docker://registry.demo.netapp.com/trident-operator:26.02.1 \
  --src-tls-verify=false --dest-tls-verify=false
fi
