...
if [[ -z "$(curl -s -u registryuser:Netapp1! 'https://registry.demo.netapp.com/v2/dbench/tags/list' | jq -r '.tags[]? | select(.=="1.0.0")')" ]]; then
  echo
  echo "##############################################################"
  ...
  podman run --rm quay.io/containers/skopeo:latest copy --dest-creds 'registryuser:Netapp1!' \
    docker://quay.io/yvosonthehub/dbench:1.0.0 docker://registry.demo.netapp.com/dbench:1.0.0 \
