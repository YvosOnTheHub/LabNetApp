...
```bash
$ kubectl run mysql-client -n mysql --image=registry.demo.netapp.com/mysql:5.7.30 -i --rm --restart=Never -- mysql -h mysql-0.mysql <<EOF
...
```bash
$ kubectl run mysql-client -n mysql --image=registry.demo.netapp.com/mysql:5.7.30 -i -t --rm --restart=Never -- mysql -h mysql-read -e "SELECT * FROM test.messages"
...
```bash
$ kubectl run mysql-client-loop -n mysql --image=registry.demo.netapp.com/mysql:5.7.30 -i -t --rm --restart=Never -- bash -ic "while sleep 1; do mysql -h mysql-read -e 'SELECT @@server_id,NOW()'; done"
+-------------+---------------------+
| @@server_id | NOW()               |
+-------------+---------------------+
