...
if [[  $(docker images | grep trident | grep 21.07.2 | wc -l) -ne 0 ]]
  then
    echo "TRIDENT 21.07.2 images already present. Nothing to do"
    exit 0
fi
...
do
  ...
  ssh -o "StrictHostKeyChecking no" root@$host docker pull netapp/trident:21.07.2
  ssh -o "StrictHostKeyChecking no" root@$host docker pull netapp/trident-operator:21.07.2
